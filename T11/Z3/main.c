...
int tacka_u_pravougaoniku(struct Tacka t, struct Pravougaonik pravougaonik){
	if(t.x>=pravougaonik.dole_lijevo.x && t.x<=pravougaonik.gore_desno.x && t.y>=pravougaonik.dole_lijevo.y && t.y<=pravougaonik.gore_desno.y) return 1;
	else return 0;
}
